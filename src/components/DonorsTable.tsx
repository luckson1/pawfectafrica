...
function DonorsTable() {
  const {data:users, isLoading, isError, error}=api.user.getAllDonors.useQuery()
  ...
  return (
        ...
        <div className="table text-left w-full">
          ...
          <li className="table-header md:rounded-md rounded-xl  my-5 md:my-2">
              ...
              <div className="col col-2 text-left">Pets Donated</div>
            ...
            {users?.map((user) => (
              <li className="table-row md:rounded-md rounded-xl   my-5 md:my-2" key={user.id}>
                ...
                <div className="col col-2" data-label="Pets" >
                <div className="flex gap-1">  {user?.Pet.length <= 0
                  ? "No Pets"
                  : user.Pet.map(pet=> (
                    <Image
